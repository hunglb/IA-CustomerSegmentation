...
class CustomerSegmentationPrep():
    ...
    nulls_threshold=0.1):
        ...
        if self.train_or_score == 'train':
          ...
          user_inputs_dict = { 'columns_required' : columns_required, 'default_attributes' : default_attributes,
    ...
    def prep_data(self, df_raw, train_or_score):
        ...
        if self.date_customer_joined in df_prep.columns:
            ...
            if df_prep.shape[0] == 0:
                print('Error: No data to train with, check the effective_date variable')
            else:
        ...
        # save or load the json file with column names that are required
        # for training, it saves out the names and order of columns that were used to build the model
        # for scoring, it loads the json, giving the column names and order that the scoring dataset needs to be in
        if train_or_score == 'train':
            # save out a list of columns that were created when the training dataset was built
            with open('../datasets/training_cols.json', 'w') as f:
                json.dump(list(df_prep.columns), f)
